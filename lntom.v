...
(* alternative proof using step lemma *)
Lemma gen_moveL_step_pr: forall V ps concl 
  (last_rule rules : rls (list (rel (list (PropF V)) * dir))),
  last_rule = nsrule (seqrule (@princrule V)) ->
  gen_moveL_step last_rule rules ps concl.
Proof.  intros until 0.  unfold gen_moveL_step.
...
intros. inversion H.
subst.
pose gen_moveL_step_pr.
unfold gen_moveL_step in g.
