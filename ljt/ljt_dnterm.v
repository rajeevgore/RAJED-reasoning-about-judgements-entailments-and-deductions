...
Inductive ms_ord {U} R : relationT (list U) :=
    ms_ordI : forall G1 G2 ps c Gsw, (forall p : U, InT p ps -> R p c) ->
      swapped (G1 ++ ps ++ G2) Gsw -> ms_ord R Gsw (G1 ++ c :: G2).

Lemma ms_ordI_eqc {U} R G1 G2 ps c Gsw G1c2 :
  (forall p : U, InT p ps -> R p c) -> swapped (G1 ++ ps ++ G2) Gsw ->
...
inversion H0. simpl. apply swapped_same.
...
inversion H1.  simpl. apply swapped_same.  Qed.
...
inversion H3. simpl. apply swapped_same.
...
inversion X. simpl. apply swapped_same. Qed.
...
+ eapply (ms_ordI_eqc _ [] []). 3: reflexivity.
2: rewrite app_nil_r. 2: apply swapped_same.
...
inversion H0.
+ eapply (ms_ordI_eqc _ [] [_]).
intros p inp. apply (InT_nilE _ inp).  apply swapped_same. reflexivity.

- inversion H0 ; subst ; clear H0.
inversion H1 ; subst ; clear H1.
apply tT_step. eapply (ms_ordI_eqc _ [_] []).
3: reflexivity.  2: rewrite app_nil_r. 2: apply swapped_same.
intros p inp.  inversion inp ; subst ; clear inp.
...
eapply (@ms_ordI _ _ [cr] [] [B]).  2: apply swapped_same.
...
all: right ; eapply (@ms_ordI _ _ [] [_] [_]) ; try (apply swapped_same) ;
