...
apply NSlclctxt'. 
rewrite app_assoc.  (* rewrite so as to identify two parts of context *)
rewrite app_assoc.
apply WBox1Ls.
ms_cgs_weak acm. 
list_assoc_r'.
  rewrite ?list_rearr22. eapply acm.
2 : unfold nslext ; unfold nslclext ; list_assoc_r' ; simpl ; reflexivity.
2 : reflexivity.
weak_tacX.
...
*)
 (*
  assert
    ...
    ((G ++ [(H1l ++ WBox A :: H1r, K1, d0)]) ++ [(A0 ++ H ++ B ++ H2r, K2, fwd)]) ->
    derrec rules (fun _ : list (rel (list (PropF V)) * dir) => False)
           ((G ++ [(H1l ++ WBox A :: H1r, K1, d0)]) ++ [(A0 ++ B ++ H ++ H2r, K2, fwd)])) as HH.
  admit.
  apply HH. clear HH.
*)
...
(* unfold can_gen_weakR in acm. *)
  assoc_mid B;
  repeat (try eapply acm; assoc_single_mid)];
   unfold nslext ; unfold nslclext ; list_assoc_r' ; simpl; reflexivity.
   list_eq_assoc.
 *)
...
Lemma gen_swapR_lc_b1l: forall {V : Set} ns
  (D : derrec (nslcrule (@b1lrules V)) (fun _ => False) ns),
  can_gen_swapR (nslcrule (@b1lrules V)) ns.
Proof. 
intro.  intro.  intro.
eapply derrec_all_ind in D.
exact D. tauto.
...
(* 
Lemma gen_weakL_step_pr: forall V ps concl 
  (last_rule rules : rls (list (rel (list (PropF V)) * dir))),
...
Admitted.
(*
  intros. eapply gen_weakL_step_loe. apply princrule_L_oe'. exact H. Qed.
 *)
...
intros. inversion H. 
subst.
eapply gen_weakL_step_pr_lc. reflexivity.
eassumption.
pose proof gen_weakL_step_pr_lc as g.
unfold gen_weakL_step in g.
eapply g.
pose gen_weakL_step_pr.
...
(*
(* To be integrated into a weak_tac to replace swap_tac. *)
Lemma weakened_lem1 : forall {T : Type} (B l Φ1 Φ2 : list T),
 weakened (Φ1 ++ l ++ Φ2) (Φ1 ++ l ++ B ++ Φ2).
Proof.
  intros. do 2 rewrite (app_assoc Φ1 l).
  apply weakened_I'. auto.
Qed.

Lemma weakened_lem2 : forall {T : Type} (B C l1 Φ1 H1 : list T),
weakened (Φ1 ++ l1 ++ H1 ++ C) (Φ1 ++ l1 ++ H1 ++ B ++ C).
Proof.
  intros. do 2 rewrite (app_assoc Φ1 l1).
  do 2 rewrite (app_assoc _ H1).
  apply weakened_I'. auto.
Qed.
*)
