...
Definition C8_case `{SL : SUBSTLLANG} (DC : DISPCALC)
  (X Y : structr) (ls : list sequent) (fmls : formula -> Prop) :=
...
Definition C8_case_alt `{SL : SUBSTLLANG} (DC : DISPCALC)
  (X Y : structr) (ls : list sequent) (fmls : formula -> Prop) :=
  deriv_cofseqs DC fmls ls -> deriv_cofseq DC fmls (X ⊢ Y).


Lemma C8_case_alt_imp_C8_case `{SL : SUBSTLLANG} (DC : DISPCALC) :
  forall X Y ls fmls, C8_case_alt DC X Y ls fmls -> C8_case DC X Y ls fmls.
Proof.
...
Ltac auto_C4 :=    
  ...
  | [ H : _ = r |- _ ] =>
    ...
    | [ H0 : _ = prems |- _ ] =>
      match goal with
      | [ H1 : _ = s |- _ ] =>
        rewrite <- H0, <- H1; unfold seqSVs'; simpl; intro; destruct_or; (discriminate || tauto)
      end
    end)
  end.
